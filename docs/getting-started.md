...
This guide installs the OpenAgents AI SDK and shows three small programs. Each
program runs against the published packages. Every symbol in this guide exists
in the `rc` train on npm.

**Source of truth.** This page is kept in sync with the monorepo consumer copy
at
[`openagents/docs/ai-sdk/getting-started.md`](https://github.com/OpenAgentsInc/openagents/blob/main/docs/ai-sdk/getting-started.md)
(live at [openagents.com/aisdk/docs/getting-started](https://openagents.com/aisdk/docs/getting-started)).
The engine lives here; the two copies must not drift.

The three flows below are verified runnable against the published `rc` train.
...
- The packages publish TypeScript source directly. Use a TypeScript-aware
  ...
  inside `node_modules`.
- The packages depend on `effect` `4.0.0-beta.94`. Keep one `effect` version in
  your application.
...
The second `start` call can run in a different process. The continuation carries
the cursor and the remaining state, so the two slices concatenate into one
contiguous stream.
...
import { Effect, Stream } from "effect";
import { makeReferenceAdapter } from "@openagentsinc/ai/harness";
import { applyUiChunk, initialUiMessage, khalaEventToUiChunks } from "@openagentsinc/ai/ui-stream";
...
- Read [Packages](./packages.md) for the per-package export map.
- Read the [layer index](./README.md) for the L0–L6 diagram and roster.
- Each layer package has its own README under
  [`packages/*/README.md`](../packages) with its primary API and one example.
