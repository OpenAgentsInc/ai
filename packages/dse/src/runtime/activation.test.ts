...
const canaryChannel = (candidate = pointer("v1")): ReleaseChannel => {
  const result = beginCanary({
    channel: shadowChannel(candidate),
    plan: canaryPlan,
    reason: "1% then 25%",
    now,
  });
...
const activeChannel = (candidate = pointer("v1")): ReleaseChannel => {
  const result = promoteActivation({
    channel: canaryChannel(candidate),
...
describe("AFS-09 gated activation resolution", () => {
  test("shadow mode serves the baseline and never substitutes the released artifact", () => {
    ...
    for (const key of ["a", "b", "c", "d", "e"]) {
      expect(resolveActivation({ channel, requestKey: key, sha256: sha256Hex })).toEqual({
        serve: "baseline",
      });
    }
  });
  ...
  test("canary serves a bounded, deterministic, sticky population", () => {
    ...
    for (let index = 0; index < total; index += 1) {
      if (
        resolveActivation({ channel, requestKey: `req-${index}`, sha256: sha256Hex }).serve ===
        "released"
      ) {
        served += 1;
      }
  ...
  test("active serves the released artifact; rolled_back serves the baseline", () => {
    ...
    expect(resolveActivation({ channel: active, requestKey: "x", sha256: sha256Hex }).serve).toBe(
      "released",
    );
  });
});
...
describe("AFS-09 activation transitions", () => {
  test("beginShadow rejects a signature mismatch", () => {
    ...
    expect(
      beginShadow({ signatureId: SIG, baseline: other, candidate: pointer("v1"), now }),
    ).toEqual({
      ok: false,
  ...
  test("promote requires canary mode; abort returns to the baseline", () => {
    const channel = shadowChannel();
    expect(promoteActivation({ channel, reason: "no", now })).toEqual({
      ok: false,
      reason: "wrong_mode",
    });
    ...
    expect(
      resolveActivation({ channel: aborted.channel, requestKey: "x", sha256: sha256Hex }),
    ).toEqual({
      serve: "baseline",
    });
  });
  ...
  test("rollback of a first release falls to the baseline without a rebuild", () => {
    ...
    expect(
      resolveActivation({ channel: rolledBack.channel, requestKey: "x", sha256: sha256Hex }),
    ).toEqual({
      serve: "baseline",
    });
  });
  ...
  test("rollback restores a prior released artifact and keeps serving it (no rebuild)", () => {
    ...
    const channelWithPrior: ReleaseChannel = {
      ...activeChannel(pointer("v2")),
      prior: priorPointer,
    };
    const rolledBack = rollbackActivation({
      channel: channelWithPrior,
      reason: "regression on v2",
      now,
    });
    if (!rolledBack.ok) throw new Error(rolledBack.reason);
    ...
    const decision = resolveActivation({
      channel: rolledBack.channel,
      requestKey: "x",
      sha256: sha256Hex,
    });
    expect(decision.serve).toBe("released");
    if (decision.serve === "released")
      expect(decision.pointer.candidateId).toBe(priorPointer.candidateId);
  });
});
