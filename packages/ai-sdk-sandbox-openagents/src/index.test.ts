...
describe("createOpenAgentsAiSdkSandboxProvider", () => {
  test("maps lifecycle, onFirstCreate, explicit homes, and snapshot identity", async () => {
    ...
    expect(await session.readTextFile({ path: "setup/result.txt" })).toBe("setup complete");
...
class FixtureOpenAgentsSandboxClient implements OpenAgentsSandboxV1Client {
  ...
  async resumeSession(input: { sessionId: string }): Promise<OpenAgentsSandboxSessionDescriptor> {
  ...
  async readBinaryFile(input: { sessionId: string; path: string }): Promise<Uint8Array | null> {
    this.requireSession(input.sessionId);
  ...
  async run(input: {
    ...
    const host = input.command.match(/^openagents-network-check (?<host>.+)$/)?.groups?.host;
    if (host !== undefined) {
