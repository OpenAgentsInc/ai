...
export class OpenAgentsAiSdkSandboxProvider implements HarnessV1SandboxProvider {
  ...
  createSession = async (
    ...
    const snapshotIdentity = buildOpenAgentsSandboxSnapshotIdentity({
      bridgeBootstrapRecipeRef: options.identity ?? "ai-sdk-harness-bootstrap.unspecified",
...
export class OpenAgentsAiSdkNetworkSandboxSession implements HarnessV1NetworkSandboxSession {
  ...
  constructor(
    ...
    this.defaultWorkingDirectory = posix.normalize(input.descriptor.defaultWorkingDirectory);
  ...
  getPortUrl = async (options: {
    ...
    if (!this.exposedPorts.includes(options.port)) {
      throw new Error(`Port ${options.port} is not exposed on OpenAgents sandbox ${this.id}.`);
    }
...
export function buildOpenAgentsSandboxSnapshotIdentity(
  ...
  const identity = createHash("sha256").update(JSON.stringify(normalizedInputs)).digest("hex");
  return {
...
function assertLaneNetworkPolicy(
  input: Readonly<{
    lane: OpenAgentsSandboxLane;
