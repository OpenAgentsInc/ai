...
describe("openagents.runtime_control_intent.v2", () => {
  test("keeps queue, steer, and interrupt distinct and ref-only", () => {
    const intents = [
      ...
      decodeRuntimeControlIntent({
        ...common,
        intentRef: "intent.desktop.thread_control.2",
        idempotencyKey: "idem.desktop.thread_control.2",
        kind: "turn.steer",
        turnRef: "turn.codex.7",
        messageRef: "message.desktop.2",
      }),
      decodeRuntimeControlIntent({
        ...common,
        intentRef: "intent.desktop.thread_control.3",
        idempotencyKey: "idem.desktop.thread_control.3",
        kind: "turn.interrupt",
        turnRef: "turn.codex.7",
      }),
    ];
  ...
  test("rejects missing kind-specific identity, raw body, and malformed refs", () => {
    ...
    expect(() =>
      decodeRuntimeControlIntent({ ...common, kind: "turn.queue", messageRef: "../escape" }),
    ).toThrow();
    expect(() =>
      decodeRuntimeControlIntent({
        ...common,
  ...
  test("classifies an exact lost-ACK retry and conflicting identity reuse", () => {
    ...
    const conflict = decodeRuntimeControlIntent({
      ...first,
      messageRef: "message.desktop.changed",
    });
