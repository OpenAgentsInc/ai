...
export const AgentDefinitionWebhookSource = S.Literals(["github", "forum", "slack"]);
export type AgentDefinitionWebhookSource = typeof AgentDefinitionWebhookSource.Type;
...
const githubSubjectRef = (payload: Record<string, unknown>, deliveryId: string): string => {
  ...
  const refPrefix =
    fullName === undefined ? "github.delivery" : `github.repository.${compactRefSegment(fullName)}`;
...
const githubSourceRefs = (
  ...
  if (fullName !== undefined && pullNumber !== undefined) {
    refs.push(`github.pull_request.${compactRefSegment(fullName)}.${pullNumber}`);
  }
...
const forumPayload = (payload: Record<string, unknown>): Record<string, unknown> => {
...
const forumSubjectRef = (payload: Record<string, unknown>, deliveryId: string): string => {
...
const slackEventRecord = (payload: Record<string, unknown>): Record<string, unknown> | undefined =>
...
const slackSubjectRef = (payload: Record<string, unknown>, deliveryId: string): string => {
  const event = slackEventRecord(payload);
  const teamId = slackTeamId(payload, event);
  const channelId = stringValue(event?.channel);
...
const slackSourceRefs = (
  ...
  if (teamId !== undefined && channelId !== undefined) {
    refs.push(`slack.channel.${compactRefSegment(teamId)}.${compactRefSegment(channelId)}`);
  }
...
export const normalizeGitHubWebhookEvent = (
  ...
  const normalizedPayload = optionalObject([
    ...
    ["pull_request", githubIssueLikePayload(asRecord(input.payload.pull_request))],
    ["mention", mention],
    ["repository", repository],
    ["sender", githubSenderPayload(asRecord(input.payload.sender))],
    ["subject", githubIssueSubjectPayload(input.payload)],
  ]);
  const baseEventType = action === undefined ? eventName : `${eventName}.${action}`;

  return {
    schema: AgentDefinitionWebhookNormalizedEventSchemaLiteral,
    source: "github",
    eventType: booleanValue(mention?.present) === true ? `${baseEventType}.mention` : baseEventType,
    deliveryId,
...
const pathSegments = (path: string): ReadonlyArray<string> | undefined => {
  ...
  return segments.every((segment) => /^[A-Za-z0-9_]+$/.test(segment)) ? segments : undefined;
};

const jsonPathValue = (root: Record<string, unknown>, path: string): unknown => {
