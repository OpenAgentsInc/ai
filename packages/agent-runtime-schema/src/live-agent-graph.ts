...
export const LiveAgentGraphRuntime = S.Union([
  S.Struct({
    state: S.Literal("known"),
    kind: S.Literals(["codex_app_server", "claude_agent_sdk", "claude_pylon", "openagents_native"]),
    runtimeRef: Ref,
  }),
  S.Struct({ state: S.Literal("unknown"), reason: UnknownReason }),
]);
...
const sameKnownIdentity = (before: LiveAgentGraphNode, after: LiveAgentGraphNode): boolean => {
  if (
...
const validateNodeTransition = (before: LiveAgentGraphNode, after: LiveAgentGraphNode): void => {
  if (!sameKnownIdentity(before, after)) {
    fail("identity_conflict", `agent ${after.agentRef} changed stable identity`);
  }
  if (after.version <= before.version)
    fail("stale_node", `agent ${after.agentRef} version did not advance`);
...
export const applyLiveAgentGraphDelta = (
  ...
  if (
    duplicate(delta.upsertNodes.map((node) => node.agentRef)) !== null ||
