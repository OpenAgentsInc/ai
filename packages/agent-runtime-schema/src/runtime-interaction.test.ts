...
describe("openagents.runtime_interaction.v1", () => {
  ...
  test("applies one exact question decision, deduplicates its retry, and rejects conflicting reuse", () => {
    ...
    expect(
      applyRuntimeInteractionDecision(applied.interaction, answer, "2026-07-11T22:02:00.000Z")
        .state,
    ).toBe("duplicate");
    expect(
      applyRuntimeInteractionDecision(
  ...
  test("rejects wrong-kind, unknown-option, incomplete, and invalid multi-select decisions", () => {
    ...
    expect(
      invalid.map(
        (decision) =>
          applyRuntimeInteractionDecision(interaction, decision, "2026-07-11T22:01:00.000Z").state,
      ),
    ).toEqual(["invalid_decision", "invalid_decision", "invalid_decision", "invalid_decision"]);
  });
  ...
  test("expires from server time and refuses both expired and revoked decisions", () => {
    ...
    const late = applyRuntimeInteractionDecision(interaction, answer, "2026-07-11T22:05:00.000Z");
    ...
    expect(
      applyRuntimeInteractionDecision(late.interaction, answer, "2026-07-11T22:06:00.000Z").state,
    ).toBe("expired");
    ...
    expect(applyRuntimeInteractionDecision(revoked, answer, "2026-07-11T22:03:00.000Z").state).toBe(
      "revoked",
    );
  });
