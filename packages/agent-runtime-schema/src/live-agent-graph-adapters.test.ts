...
import { LiveAgentGraphSchemaLiteral, validateLiveAgentGraphSnapshot } from "./live-agent-graph.js";
...
const codex = (
  ...
  agent: {
    ...
    updatedAt,
    startedAt: createdAt,
    endedAt: ["completed", "failed", "cancelled", "interrupted"].includes(status)
      ? updatedAt
      : null,
    version: 3,
  },
...
const claude = (
  ...
  agent: {
    ...
    updatedAt,
    startedAt: createdAt,
    endedAt: ["succeeded", "errored", "cancelled", "interrupted"].includes(status)
      ? updatedAt
      : null,
    version: 3,
  },
...
const semantics = (value: ReturnType<typeof adaptCodexLiveAgentObservation>) => ({
  ...
  currentTool:
    value.node.currentTool.state === "known"
      ? {
          state: "known",
          toolName: value.node.currentTool.toolName,
          status: value.node.currentTool.status,
        }
