...
const parentEdgeFor = (
  ...
  threadId: string,
  parent: typeof ParentObservation.Type,
): typeof LiveAgentGraphEdge.Type | null =>
  parent.state !== "known"
    ? null
    : {
...
const canonicalNode = (
  ...
  return S.decodeUnknownSync(LiveAgentGraphNode)({
    ...
    provider:
      input.providerFact.state === "known"
...
const claudeStatus = (
  status: ClaudeLiveAgentObservation["agent"]["status"],
): LiveAgentGraphStatus => CLAUDE_STATUS[status];

const codexTool = (
  threadId: string,
  tool: CodexLiveAgentObservation["agent"]["currentTool"],
): Readonly<{
  current: LiveAgentGraphCurrentTool;
  edge: typeof LiveAgentGraphEdge.Type | null;
}> => {
  ...
  const status =
    tool.status === "pending" ? "called" : tool.status === "inProgress" ? "running" : tool.status;
  return {
...
export const adaptCodexLiveAgentObservation = (value: unknown): AdaptedLiveAgentObservation => {
  ...
  return {
    ...
    edges: [parentEdge, tool.edge].filter(
      (edge): edge is typeof LiveAgentGraphEdge.Type => edge !== null,
    ),
  };
};
