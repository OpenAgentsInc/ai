...
const githubMentionCommentPayload = {
  ...
  comment: {
    ...
    body: "@OpenAgents please run the bounded definition.\n\nPrivate-ish freeform text stays out.",
    html_url: "https://github.com/OpenAgentsInc/openagents/issues/8209#issuecomment-1001",
    id: 1001,
    user: {
      id: 790,
      login: "AtlantisPleb",
...
const forumPostPayload = {
  ...
  postNumber: 3,
  postState: "visible",
  sourceUrl: "https://openagents.com/forum/t/topic_forum_trigger_001#post_forum_trigger_001",
...
const slackMessagePayload = {
  ...
  event: {
    ...
    text: "Secret-ish Slack message body must never become trigger payload or ledger material.",
...
describe("agent definition webhook normalization", () => {
  test("normalizes GitHub issue events into a typed bounded payload", () => {
    ...
    expect(event?.sourceRefs).toContain("github.issue.OpenAgentsInc/openagents.8195");
  });
  ...
  test("normalizes GitHub issue-comment mentions without raw comment bodies", () => {
    ...
    expect(JSON.stringify(event?.payload)).not.toContain("Private-ish freeform text stays out");
  });
  ...
  test("keeps unmentioned GitHub issue comments out of mention triggers", () => {
    ...
    expect(
      agentDefinitionWebhookConditionsMatch(event!, [
        {
          kind: "event_type",
  ...
  test("normalizes Forum post events into bounded public source refs", () => {
    ...
    expect(event).toMatchObject({
      ...
      eventType: "forum.post.created",
      deliveryId: "forum-delivery-123",
      subjectRef: "forum.topic.topic_forum_trigger_001.post.post_forum_trigger_001",
      payload: {
  ...
  test("normalizes Slack message events into bounded refs without raw text", () => {
    ...
    expect(event).toMatchObject({
      ...
      eventType: "message",
      deliveryId: "Ev_BACKGROUND_8214",
      subjectRef: "slack.team.T_OPENAGENTS.channel.C_BACKGROUND.message.1783152000.000100",
      payload: {
        actor: {
          user_id: "U_OWNER",
        },
