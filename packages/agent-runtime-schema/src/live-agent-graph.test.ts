...
const at = (minute: number): string => `2026-07-11T19:${String(minute).padStart(2, "0")}:00.000Z`;
...
const parentEdge = (parentRef: string, childRef: string, version = 1): LiveAgentGraphEdge => ({
...
describe("canonical live agent graph v1", () => {
  ...
  test("adds a child atomically with one stable parent edge", () => {
    ...
    const next = applyLiveAgentGraphDelta(
      snapshot(),
      delta({
        upsertNodes: [child],
  ...
  test("exact replay is idempotent while gaps, conflicts, and stale generations fail", () => {
    ...
    expect(
      errorReason(() =>
        applyLiveAgentGraphDelta(current, {
          ...first,
    ...
    expect(
      errorReason(() =>
        applyLiveAgentGraphDelta(
          ...
          delta({
            upsertNodes: [
              node({ agentRef: "agent.root", version: 2, cursor: 3, updatedAt: at(2) }),
            ],
          }),
        ),
      ),
  ...
  test("rejects missing parents, mismatched edges, orphan tools, and cycles", () => {
    ...
    expect(errorReason(() => validateLiveAgentGraphSnapshot(snapshot([child])))).toBe(
      "missing_parent",
    );
    expect(
      errorReason(() =>
    ...
    const left = node({
      agentRef: "agent.left",
      parent: { kind: "agent", agentRef: "agent.right" },
    });
    const right = node({
      agentRef: "agent.right",
      parent: { kind: "agent", agentRef: "agent.left" },
    });
    expect(
      errorReason(() =>
        validateLiveAgentGraphSnapshot(
  ...
  test("permutation property: one atomic child batch converges byte-identically", () => {
    ...
    const expected = applyLiveAgentGraphDelta(
      snapshot(),
      delta({
    ...
    for (let seed = 1; seed <= 50; seed += 1) {
      const candidate = applyLiveAgentGraphDelta(
        snapshot(),
        decodeLiveAgentGraphDelta(
          delta({
