...
export function lowerKhalaAiSdkProviderOptions(
  input: Readonly<{
  ...
  const existing = isRecord(merged[providerKey]) ? merged[providerKey] : {};
...
export async function runKhalaAiSdkCoreRuntime(
  ...
  const result = await callStreamText(buildKhalaAiSdkCoreStreamTextOptions(input));
  return collectKhalaAiSdkCoreEventsFromStream({
    source: input.source ?? sourceFromProvider(input.provider),
    stream: result.stream,
    threadId: input.threadId,
    turnId: input.turnId,
    ...(input.eventVisibility === undefined ? {} : { eventVisibility: input.eventVisibility }),
    ...(input.observedAt === undefined ? {} : { observedAt: input.observedAt }),
    ...(input.onPrivateRawPart === undefined ? {} : { onPrivateRawPart: input.onPrivateRawPart }),
    ...(input.rawMode === undefined ? {} : { rawMode: input.rawMode }),
    ...(input.toolAuthority === undefined ? {} : { toolAuthority: input.toolAuthority }),
  });
}

export async function collectKhalaAiSdkCoreEventsFromStream(
  input: Readonly<{
    stream: AsyncIterable<unknown>;
    threadId: string;
    turnId: string;
  ...
  for await (const rawPart of input.stream) {
    ...
    if (part.type !== "raw" || input.rawMode !== "discard") {
      ...
      const mapped = khalaRuntimeEventFromAiSdkTextStreamPart({
        eventId: `event.${stableRef([input.threadId, input.turnId, String(sequence), part.type])}`,
      ...
      events.push(withVisibility(mapped, input.eventVisibility ?? "private"));
      sequence += 1;
    }
  }
  ...
  for (const event of events) {
    switch (event.kind) {
      ...
      case "turn.finished":
        ...
        break;
      case "text.delta":
        textByMessageId[event.messageId] = (textByMessageId[event.messageId] ?? "") + event.text;
        break;
...
export function reduceAgentRuntimeEventsAsKhalaTranscript(
  input: Readonly<{
...
export function normalizeAiSdkTextStreamPart(part: unknown): KhalaRuntimeAiSdkTextStreamPart {
...
function toolNameFromPart(part: KhalaRuntimeAiSdkTextStreamPart): string | undefined {
  switch (part.type) {
    case "tool-call":
    case "tool-result":
    case "tool-error":
...
function sourceFromProvider(provider: KhalaAiSdkProviderProfile | undefined): KhalaRuntimeSource {
  return {
    lane: "ai_sdk_core",
    surface: "server",
...
function withVisibility(
  ...
  return visibility === "public" ? assertKhalaRuntimePublicEventSafe(next) : next;
}
...
function safeRefSegment(value: string): string {
  return /^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(value) ? value : stableRef([value]);
}
