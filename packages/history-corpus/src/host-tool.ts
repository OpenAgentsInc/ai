...
export interface HistoryRecallHostToolParams extends S.Schema.Type<
  typeof HistoryRecallHostToolParams
> {}
...
export const historyRecallToolkitHandlers = (recall: HistoryRecallShape) =>
  HistoryRecallToolkit.of({
    history_recall: (params) =>
      ...
      recall.recall(
        historyRecallRequestFromHostParams(params),
      ) as Effect.Effect<HistoryRecallResponseType>,
  });
...
export const summarizeHistoryRecallAnswer = (
  ...
  const more =
    answer.answers.length > maxCitations ? ` (+${answer.answers.length - maxCitations} more)` : "";
