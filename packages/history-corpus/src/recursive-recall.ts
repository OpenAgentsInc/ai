...
export const renderRecursiveRecallEntryLine = (
  index: number,
...
export const cursorSliceRecursiveRecallCorpus = (
  ...
  for (let index = 0; index < entries.length; index++) {
    ...
    if (
      entry.turnId === turnId &&
      entry.sequence >= sequenceStart &&
      entry.sequence <= sequenceEnd
    ) {
...
export const runRecursiveRecall = (
      ...
      for (let iteration = 1; iteration <= options.caps.maxIterations; iteration++) {
        ...
        switch (op._tag) {
          ...
          case "Subcall": {
            ...
            if (
              child._tag === "Partial" &&
              (child.reason === "token_cap" || child.reason === "subcall_cap")
            ) {
  ...
  const run = loop(options.corpus, options.question, 0).pipe(
    ...
    Effect.map((outcome) => (Option.isSome(outcome) ? outcome.value : partialOf(state, "timeout"))),
  );
