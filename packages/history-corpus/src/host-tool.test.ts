...
const scriptTurn = (turnId: string, words: ReadonlyArray<string>): Array<HarnessStreamEvent> => {
...
describe("history_recall host tool (RLM-03)", () => {
  ...
  test("dispatch emits neutral tool.call + tool.result re-entry", async () => {
    ...
    expect(dispatched.neutralEvents.map((e) => e.kind)).toEqual(["tool.call", "tool.result"]);
  ...
  test("error path emits tool.call + tool.error on the neutral stream", async () => {
    ...
    expect(dispatched.neutralEvents.map((e) => e.kind)).toEqual(["tool.call", "tool.error"]);
  });
});
